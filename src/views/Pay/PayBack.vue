...
const payMoneyText = computed(() => {
  const v = orderInfo.value && orderInfo.value.payMoney
  return typeof v === 'number' ? v.toFixed(2) : '0.00'
})

function goOrder() {
  const id = route.query.id || route.query.orderId || orderInfo.value.id
  // 跳转到我的订单页，可传 order id
  router.push({ path: '/member/order', query: { id } })
}
  ...
  <div class="xtx-pay-page">
    <div class="container">
      ...
      <div class="pay-result">
        ...
        <p>
          支付金额：
          <span>¥{{ payMoneyText }}</span>
        </p>
        <div class="btn">
          <el-button type="primary" style="margin-right: 20px" @click="goOrder">查看订单</el-button>
