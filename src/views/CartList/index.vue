...
const allCount = computed(() => {
  return cartStore.cartList.reduce((sum, item) => sum + (item.count || 0), 0)
})

const selectedCount = computed(() => {
  return cartStore.cartList.reduce((sum, item) => sum + ((item.selected ? item.count : 0) || 0), 0)
})

const selectedPrice = computed(() => {
  return cartStore.cartList.reduce((sum, item) => {
    return sum + ((item.selected ? item.price * item.count : 0) || 0)
  }, 0)
})
      ...
      <div class="cart">
        <table>
          <thead>
            <tr>
              <th width="120">
                <el-checkbox :model-value="cartStore.cartList.length > 0 && cartStore.cartList.every(item => item.selected)" @change="(checked) => allCheckChange(checked)" />
              </th>
