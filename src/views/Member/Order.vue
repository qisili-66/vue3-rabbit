...
import { getMemberOrderListAPI } from '@/apis/member'

const tabTypes = [
  { name: 'all', label: '全部' },
  { name: 'pending', label: '待付款' },
  { name: 'delivered', label: '待收货' },
...
const fetchOrders = async () => {
  try {
    ...
    orderList.value = result.records || result.orders || []
    total.value = result.total || orderList.value.length
  } catch (err) {
...
const tabChange = (tab) => {
  ...
  params.value.status = map[tab.name] !== undefined ? map[tab.name] : ''
  params.value.page = 1
  fetchOrders()
}
